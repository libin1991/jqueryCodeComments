...
var
	...
	// selector        jquery 用户所写的内容 可以是字符串 null  function 等 例如 $(function(){}) $("#id")
 	// context         上下文环境 一般是document
	jQuery = function( selector, context ) {

		// The jQuery object is actually just the init constructor 'enhanced'
		// Need init if jQuery is called (just allow error to be thrown if not included)
		//jquery.fn 指向 jQuery.prototype  jQuery.fn.init 的 原型 指向 jQuery.prototype  这样来实现外部无NEW的方式
...
//jquery 构建的对象是类似数组的对象 所以他会有一个length  下标 对应所选择到的dom  类似 {0：dom,1:dom,2:dom,length:3}
...
function isArrayLike( obj ) {
	...
	//!!obj 其实可以防止用户传入0 这样可以保证如果只传入一个对象 并且这个对象没有长度的时候 length 只是一个boolean类型 而不是一个空对象或者0
	//下面return 会对length有个类型判断
